...
def update(chat_id, chat_type, user_id=101):
    ...
    reply = SimpleNamespace(message_id=777, edit_reply_markup=AsyncMock(), delete=AsyncMock())
    message = SimpleNamespace(reply_text=AsyncMock(return_value=reply), delete=AsyncMock())
...
async def test_group_command_issues_bound_link_without_game_rng(temp_database, monkeypatch):
    ...
    async def assert_bound_before_button(**kwargs):
        with database.get_db() as conn:
            assert conn.execute(
                "SELECT launch_message_id FROM miniapp_launch_tokens",
            ).fetchone() == (777,)

    sent.edit_reply_markup.side_effect = assert_bound_before_button
    ...
    reply = request.message.reply_text.await_args_list[0]
...
async def test_launch_response_survives_command_deletion_failure(temp_database):
    ...
    async def attach_button(**kwargs):
        events.append("attach_button")
    ...
    reply.edit_reply_markup.side_effect = attach_button
    request.message.delete.side_effect = fail_delete
    context = SimpleNamespace(bot=SimpleNamespace(username="ExampleBot"))

    await duel_app_command(request, context)

    assert events == ["reply", "attach_button", "delete_command"]
    request.message.delete.assert_awaited_once_with()
    reply.delete.assert_not_awaited()
    button = reply.edit_reply_markup.await_args.kwargs["reply_markup"].inline_keyboard[0][0]
