...
@pytest.mark.asyncio
async def test_duel_list_denies_no_dick_at_zero_before_selection_or_rng(
    ...
    assert update.message.reply_text.await_args.args[0] == get_text("duel.command.no_dick")
    assert CHAT_ID not in duel.ACTIVE_DUELS


@pytest.mark.asyncio
async def test_old_selection_button_rechecks_current_db_and_allows_zero_points(
    temp_database, fake_context, monkeypatch,
):
    import database
...
@pytest.mark.asyncio
async def test_completed_dig_from_exactly_ten_points_allows_duel_list(
    temp_database, fake_context, monkeypatch,
):
    import database
    ...
    markup = duel_update.message.reply_text.await_args.kwargs["reply_markup"]
    assert markup.inline_keyboard[0][0].callback_data == "start_duel_opponent"
    assert CHAT_ID not in duel.ACTIVE_DUELS
